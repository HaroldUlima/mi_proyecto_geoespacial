...
<head>
<meta charset="utf-8">
<title>Dashboard Geoespacial — ATMs BBVA</title>
...
<header>
    <h1>Mapa Geoespacial — ATMs BBVA</h1>
    <a href="/logout" style="
...
function getIcon(ubic, promedio){
    ...
    if(ubic.includes("OFICINA")){
        return L.divIcon({html:"<div style='font-size:32px;'>🏦</div>", iconSize:[32,32]});
    }
    if(ubic.includes("ISLA")){
        return L.divIcon({html:"<div style='font-size:32px; color:deepskyblue;'>🌐</div>", iconSize:[32,32]});
    }
...
async function fetchAndRender(){
    ...
    data.forEach(pt=>{
        ...
        <div style="font-size:13px;">
            <b>${pt.nombre}</b><br>
            <b>ATM:</b> ${pt.atm}<br>
            <b>Dirección:</b> ${pt.direccion}<br><hr>
            <b>División:</b> ${pt.division}<br>
            <b>Tipo:</b> ${pt.tipo}<br>
            <b>Ubicación:</b> ${pt.ubicacion}<br><hr>
